# Text-to-Speech: output formats (TypeScript, native REST)

The same as the [SDK output-formats recipe](../../sdk/output-formats), but calling the
REST API directly with `fetch` — control the codec, sample rate, and bitrate via the
...
- Loops over a small array of `{ format, filename }` and `POST`s to
